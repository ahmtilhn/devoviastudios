...
const files = {
  ...
  homeQa: 'src/redesign/devovia-motion-v4-qa.css',
  ...
  internalQa: 'src/ui/full-site-motion-v4-qa.css',
...
check('Homepage QA isolation stylesheet is active', contents.homeCss.includes("devovia-motion-v4-qa.css"), files.homeCss);
...
check('Internal QA stylesheet is active', contents.internalEntry.includes("full-site-motion-v4-qa.css"), files.internalEntry);
...
check('Homepage layers are explicitly isolated', contents.homeQa.includes('.dv-site > .dv-motion-v4'), files.homeQa);
check('Internal progress line uses scroll progress', contents.internalQa.includes('scaleX(var(--ux-scroll-progress))'), files.internalQa);
